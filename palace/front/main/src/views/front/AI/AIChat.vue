<template>
    <div class="AI" ref="AIBar">
        <div class="left">
            <!--             AI图标-->
            <div class="Aicon"><i class="iconfont icon-chatgpt" @click="GoAiChat"></i></div>
            <i>MY-CHAT </i>
            <i class="span"></i>
            ...
            <!--            AI绘图路由跳转-->
            <div class="AIsetting" @click="GoAiPaint">
                <i class="iconfont icon-shezhi"></i>
            </div>
        </div>
        <router-view></router-view>
        <div v-if="$route.path == '/front/aiChat'" class="newChat">
            ...
            <div class="chat hide_scrollbar" @scroll="stopTimmer">
                <li  v-for="(item , index) in userChats" :index="index" class="chat-history-item">
                    ...
                    <a :href="`#mes${index}`" class="chat-history-message">{{ item }}...</a>
        ...
        <div v-if="$route.path == '/front/aiChat'" class="main">
            ...
            <div class="post">
                <textarea v-model="newMessage" placeholder="请输入你的问题" type="text"/>
                <i @click="sendMessage" class="iconfont icon-send" v-if="!isSend"></i>
                <div class="luyin">
                    <AiConversations :isload="isSend" @update-load="onUpdateParentValue" :IconSize="`2vw`"/>
                </div>
...
import AiConversations from "@/views/front/AI/AiConversations.vue";
...
export default {
    ...
    methods: {
        ...
        goChat() {
            ...
            const sidebarLinks1 = document.querySelectorAll('.chat-history-item');
            ...
            if (this.newMessage.trim() !== '') {
                ...
                test({
                    ...
                    response => {
                        ...
                        this.$set(this.userChats, L, this.newMessage.slice(0, 8));
                        // 绑定事件
                        this.$nextTick(() => {
                            this.goChat()
                            this.startTimmer()
        ...
        async getHistoryData(id) {
            hisData({
                ...
                for (let i = this.ChatLength; i < res.data.length; i++) {

                    this.Gmessages.push(`ChatGPT: ${res.data[i].answer}`)
                    this.messages.push(`You: ${res.data[i].userMessage}`)
                    this.userChats.push((` ${res.data[i].userMessage}...`).slice(0, 8))
                    //
                }
                this.ChatLength = this.userChats?.length - 1;
        ...
        async getHistoryData1(id) {
            hisData({
                ...
                this.Gmessages.push(`ChatGPT: ${res.data[9].answer}`)
                this.messages.push(`You: ${res.data[9].userMessage}`)
                this.userChats.push((` ${res.data[9].userMessage}...`).slice(0, 8))
        ...
        //    路由跳转到绘图页面
        GoAiPaint() {
            this.$router.push("/front/aiChat/AiPaint")
        },
        GoAiChat() {
            this.$router.push("/front/aiChat")
        },
        onUpdateParentValue(newValue) {

            if (newValue == false) {
                this.getHistoryData1(this.UserId)
                setTimeout(() => {
                    this.startTimmer()
                    this.goChat()
                }, 40)
